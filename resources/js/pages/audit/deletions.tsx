...
import { ChevronLeft, ChevronRight, Mail, RotateCw, Trash2 } from 'lucide-react';
...
export default function AuditDeletions({
    deletions,
}: {
    deletions: DeleteHistoryRow[];
}) {
    ...
    const filteredDeletions = useMemo(() => {
        ...
        return deletions.filter((deletion) => {
            const typeMatches =
                typeFilter === 'All' ||
                formatEntityType(deletion.entity_type).toLowerCase() ===
            ...
            return [
                ...
                deletion.target_code,
                deletion.target_name,
                formatDateTime(deletion.deleted_at),
            ]
                .join(' ')
                .toLowerCase()
                .includes(term);
        });
    }, [deletions, search, typeFilter]);

    const totalPages =
        itemsPerPage === 100
            ? 1
            : Math.max(1, Math.ceil(filteredDeletions.length / itemsPerPage));
    const safePage = Math.min(currentPage, totalPages);
    const startIndex = itemsPerPage === 100 ? 0 : (safePage - 1) * itemsPerPage;
    const paginatedDeletions =
        itemsPerPage === 100
            ? filteredDeletions
            : filteredDeletions.slice(startIndex, startIndex + itemsPerPage);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Deletion History" />

            <div className="space-y-4 p-4 md:p-6">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="text-sm font-medium text-muted-foreground">
                        {filteredDeletions.length} total
                    </div>
                                ...
                                <div className="grid gap-4 lg:grid-cols-[1fr_0.5fr_1fr_0.5fr_0.5fr] lg:items-center">
                                    <div className="flex min-w-0 items-center gap-3">
                                        <Avatar className="size-10">
                                            <AvatarImage
                                                src={deletion.avatar_url ?? undefined}
                                                alt={deletion.name || 'User avatar'}
                                            />
                                            <AvatarFallback className="text-xs">
                                                {getInitials(deletion.name)}
                                            </AvatarFallback>
                                        </Avatar>
                                    ...
                                    <div className="min-w-0 space-y-1 text-left">
                                        ...
                                        <div className="font-medium">
                                            {formatEntityType(deletion.entity_type)}
                                        </div>
                                    </div>
