...
import { ChevronLeft, ChevronRight, LogIn, Mail, RotateCw } from 'lucide-react';
...
const formatLoginAt = (value: string | null): string => {
    if (!value) {
        return 'N/A';
    }
...
export default function AuditLogins({
    logins,
}: {
    ...
    const filteredLogins = useMemo(() => {
        ...
        return logins.filter((login) => {
            const roleMatches =
                roleFilter === 'All' ||
                formatRole(login.role).toLowerCase() === roleFilter.toLowerCase();
            ...
            return [login.name, login.role, login.email, formatLoginAt(login.created_at)]
                .join(' ')
                .toLowerCase()
                .includes(term);
        });
    }, [logins, roleFilter, search]);

    const totalPages =
        itemsPerPage === 100
            ? 1
            : Math.max(1, Math.ceil(filteredLogins.length / itemsPerPage));
    const safePage = Math.min(currentPage, totalPages);
    const startIndex = itemsPerPage === 100 ? 0 : (safePage - 1) * itemsPerPage;
    const paginatedLogins =
        itemsPerPage === 100
            ? filteredLogins
            : filteredLogins.slice(startIndex, startIndex + itemsPerPage);
                                ...
                                <div className="grid gap-4 lg:grid-cols-[1fr_0.5fr_1fr_0.5fr_0.5fr] lg:items-center">
                                    <div className="flex min-w-0 items-center gap-3">
                                        <Avatar className="size-10">
                                            <AvatarImage
                                                src={login.avatar_url ?? undefined}
                                        ...
                                        <div className="min-w-0 space-y-1">
                                            ...
                                            <div className="flex items-center gap-2 text-sm text-muted-foreground min-w-0">
                                                ...
                                                <span className="truncate">{login.email || 'N/A'}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="min-w-0 space-y-1 text-left">
                                        ...
                                        <div className="font-medium">
                                            {formatLoginAt(login.created_at)}
                                        </div>
                                    </div>
                                </div>
                            </div>
